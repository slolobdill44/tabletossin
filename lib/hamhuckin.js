...
// Master gameplay speed multiplier. 1 = real time, 2 = the classic shipped
...
function gameStart() {
  ...
  // debris from missed shots (resting at y ≈ 545-565, inside the falling
  // band, with jittering velocity) and the fresh hammo on the whacker stay
  // untagged, so they can't trip the fall detector: only a previously-
  // on-table piece falling off ends the bonus. Misses don't.
  ...
  Events.on(engine, "afterUpdate", function() {
    ...
    if (bonusPeakOnScreen !== null) {
      for (var fi = 0; fi < hammos.length; fi++) {
        ...
        // is well past 1 by the time it enters the band.
