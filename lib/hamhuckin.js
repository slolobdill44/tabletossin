...
function gameStart() {
  ...
  document.addEventListener('keydown', function(e) {
    if (!(awaitingBonusEndContinue || awaitingRoundEndContinue)) return;
    // Swallow the key so the spacebar doesn't ALSO trigger a whacker pullback
  ...
  // Round-end (non-bonus) path. Routes through the round-end overlay, which
  // waits for the player to press a key / tap before continuing. The overlay
  // itself owns the totalScore update and the route to next-level / game-complete.
