...
function gameStart() {
  ...
  // localStorage is best-effort everywhere in this file (private browsing and
  // file:// contexts can throw on access), so every read/write goes through
  // these: reads fall back to a default, writes fail silently and the value
  // simply lives for this page load only.
  ...
  // Every tappable control needs the click handler plus a touchend copy that
  // preventDefaults (so mobile doesn't also fire a synthetic click 300ms
  // later, and the tap doesn't reach the whacker's touch handlers).
  ...
  // Restart the CSS animations on an element whose keyframes are gated by a
  // class: removing/re-adding alone is coalesced by the browser, so the
  // layout read in between is what actually forces the restart.
  function restartAnimation(el) {
    void el.offsetWidth;
  }

  // GET/POST JSON with a non-2xx status treated as a failure, so callers can
  // .catch() their way to the localStorage fallback.
  ...
  var runScores = {};
  var bestScores = storageGetJSON(BEST_SCORES_KEY, {});
  function recordLevelScore(levelIdx, score) {
    ...
    if (!(key in bestScores) || score > bestScores[key]) {
      ...
      storageSetJSON(BEST_SCORES_KEY, bestScores);
    }
  }
  ...
  function showLevelIntro(levelIdx, scoreLine, preReveal, lastLevelScore) {
    ...
    restartAnimation(levelIntroElement);
  ...
  // The HUD overlays get inline display set here rather than in CSS, because
  // startLevel sets display:block inline and a stylesheet rule could not
  // override that.
  function setHudVisible(visible) {
    var display = visible ? 'block' : 'none';
    shotsText[0].style.display = display;
    scoreText[0].style.display = display;
  }

  // The two HUD copies (in-canvas scoreboard art and the mobile header) always
  // show the same running total.
  function setScoreDisplay(value) {
    scoreNumberText[0].innerText = value;
    scoreNumberText[1].innerText = value;
  }

  // Both end-of-level overlays show the same two lines.
  function setOverlayScoreLines(scoreEl, totalEl, levelScore) {
    scoreEl.innerText = 'Level score: ' + levelScore;
    totalEl.innerText = 'Total Score: ' + (totalScore + levelScore);
  }

  // Shared tail of every level-completion path: clear the play field, bank the
  // level score, and route to the next level's intro — or to the
  // game-complete screen once the last level is done.
  function finishLevelTransition(levelScore, resumeRunner) {
    setHudVisible(false);
    World.clear(engine.world, false);
    hammos = [];
    if (resumeRunner) Runner.run(runner, engine);
  ...
  function startBonusEndSequence(fh) {
    ...
    // Hide HUD overlays so the bonus-end text reads cleanly; the next level
    ...
    gameOver = true;
    setScoreDisplay(totalScore + liveScore);
    hideBonusTimer();
    bonusEndPendingScore = liveScore;
    recordLevelScore(currentLevel, liveScore);

    setTimeout(function() {
      setOverlayScoreLines(bonusEndScoreEl, bonusEndTotalEl, liveScore);
      restartAnimation(bonusEndElement);
  ...
  function advanceFromBonusEnd() {
    ...
    // Resume the Runner so the next-level intro's preReveal can spawn fresh
    // state. The HUD stays hidden until startLevel re-shows it, so it can't
    // flash over the level-intro animation.
    finishLevelTransition(liveScore, true);
  }
  ...
  function startRoundEndSequence(levelScore) {
    ...
    setOverlayScoreLines(roundEndScoreEl, roundEndTotalEl, levelScore);
    restartAnimation(roundEndElement);
  ...
  function advanceFromRoundEnd() {
    ...
    finishLevelTransition(liveScore, false);
  }
  ...
  onTap(bonusEndElement, advancePending);
  onTap(roundEndElement, advancePending);
  ...
  function endLevel(thisLevelScore) {
    ...
    setTimeout(function() {
      ...
      finishLevelTransition(thisLevelScore, false);
    }, 800);
  }
  ...
  onTap(document.getElementById('start-game-btn'), startGameFromTitle);
  ...
  function makeTableLeg(x, height) {
    return Matter.Bodies.rectangle(x, 572, 20, height, {
                        friction: 1,
                        render: { sprite: { texture: 'assets/tableleg.png'}, lineWidth: 0 }
                    });
  }
  var leftLeg = makeTableLeg(TABLE_CENTER_X - TABLETOP_WIDTH / 2 + LEG_INSET, 110);
  var rightLeg = makeTableLeg(TABLE_CENTER_X + TABLETOP_WIDTH / 2 - LEG_INSET, 130);
  ...
  if (typeof window.sessionHighScore === 'undefined') {
    window.sessionHighScore = parseInt(storageGet(HIGH_SCORE_KEY, ''), 10) || 0;
  }
  ...
  // Cumulative per-run state. `sessionHighScore` and `bestScores` deliberately
  // survive a reset — they span the whole page load.
  ...
  onTap(document.getElementById('play-again-btn'), restartFromGameComplete);
  onTap(document.getElementById('main-menu-btn'), returnToMainMenu);
  ...
  function requestSessionToken() {
    ...
    fetchJSON(SESSION_API, { method: 'POST' })
      .then(function(data) { sessionToken = (data && data.token) || null; })
      .catch(function() { sessionToken = null; });
  }

  var localEntries = storageGetJSON(LEADERBOARD_KEY, []);
  ...
  function localLeaderboardSubmit(entry) {
    ...
    storageSetJSON(LEADERBOARD_KEY, localEntries);
    return localEntries;
  }

  function leaderboardLoad() {
    if (!window.fetch) return Promise.resolve(localEntries);
  ...
  function leaderboardSubmit(entry) {
    ...
    if (!window.fetch || !sessionToken) return Promise.resolve(localEntries);
  ...
  onTap(document.getElementById('title-leaderboard-btn'), openTitleLeaderboard);
  onTap(document.getElementById('title-leaderboard-close'), closeTitleLeaderboard);
  ...
  // Put a bonus shot on the whacker: one extra toss, tagged `_bonusShot` so a
  // miss ends the bonus, with the baseline re-locked so pieces that landed
  // since the last award are covered.
  function awardBonusShot() {
    shotCount = 1;
    lastShotTime = null;
    hammo = spawnHammo();
  ...
  Events.on(engine, "afterUpdate", function() {
    ...
    var score = calcScore();
    setScoreDisplay(totalScore + score);
    ...
    if (shotCount === 0 && lastShotTime) {
      if (bonusPeakOnScreen !== null) {
        ...
        if (settledScore >= BONUS_THRESHOLD) {
          ...
          gameOver = true;
          setScoreDisplay(totalScore + settledScore);
  ...
  (function applyMobileScale() {
    var wrapper = canvasWrapper;  // #canvas-wrapper, looked up above
