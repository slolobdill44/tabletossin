...
function gameStart() {
  ...
  function showGameComplete() {
    ...
    // Leaderboard is disabled for now (server backend only works locally) —
    // see the commented-out "Leaderboard" block below.
  ...
  function startGameFromTitle() {
    titleScreen.style.display = 'none';
    // requestSessionToken();  // leaderboard disabled — see block below
  ...
  function updateHighScore() {
    ...
    try {
      window.localStorage.setItem(HIGH_SCORE_KEY, String(totalScore));
  ...
  function restartFromGameComplete() {
    // Leaderboard name-entry guard removed along with the (disabled)
    // leaderboard feature — see the commented-out block below.
  ...
  // --- Leaderboard -----------------------------------------------------------
  // Disabled for now: the server backend (api/ + Neon DB) only works when run
  // locally via `vercel dev` and isn't wired up on the deployed site, so the
  // whole feature is commented out rather than shipped half-working. The
  // corresponding UI (leaderboard buttons/panels) is hidden in index.html.
  // To re-enable: uncomment this block plus the three call sites above
  // (setUpLeaderboardPanel, and the two requestSessionToken calls), and
  // un-hide the markup in index.html.
  /*
  ...
  function localLeaderboardSubmit(entry) {
    ...
    try {
      window.localStorage.setItem(LEADERBOARD_KEY, JSON.stringify(localEntries));
    } catch (e) {
