...
function gameStart() {
  ...
  function showLevelIntro(levelIdx, scoreLine, preReveal, lastLevelScore) {
    ...
    if (typeof lastLevelScore === 'number') {
      levelIntroLastScore.innerText = 'Last level score: ' + lastLevelScore;
      levelIntroLastScore.style.display = '';
    } else {
      levelIntroLastScore.innerText = '';
      levelIntroLastScore.style.display = 'none';
    }
  ...
  // Global input listeners that advance the bonus-end overlay. They no-op if
  // the overlay isn't waiting for input, so they don't interfere with normal
  // gameplay keyboard/touch handlers.
  bonusEndElement.addEventListener('click', advanceFromBonusEnd);
  bonusEndElement.addEventListener('touchend', function(e) {
    e.preventDefault();
    advanceFromBonusEnd();
  }, { passive: false });
  document.addEventListener('keydown', function(e) {
    if (e.keyCode === 32 && awaitingBonusEndContinue) {
      e.preventDefault();
      advanceFromBonusEnd();
    }
  });
