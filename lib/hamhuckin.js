...
function gameStart() {
  ...
  // Step the engine an extra time per frame so physics runs at the speed/feel
  // that was previously achieved by accidentally running two Runners in
  // parallel (via Engine.run + Runner.run). Each step uses Matter's natural
  // ~16.67ms timestep, so constraint stability is preserved.
  ...
  // Bonus-end freeze sequence state. Runs as a chain of setTimeouts kicked
  // off when the falling hammo is detected:
  //   A. Stop Runner, start canvas zoom transition + draw red circle
  //   B. Hold the zoomed view briefly
  //   C. Snap zoom back, show bonus splash (covers screen at z-160)
  //   D. Show next-level intro behind bonus splash, fade bonus splash away
  var fallingHammo = null;
  var ZOOM_DURATION_MS   = 2200;  // MUST match `transition: transform Xs` on
                                   // `#canvas-wrapper canvas` in style.css
  var POST_ZOOM_PAUSE_MS = 500;
  var mobileScale = null;  // captured by applyMobileScale
  ...
  function showBonusEndedSplash(levelScore, onHoldDone) {
    bonusEndScoreEl.innerText = 'Level score: ' + levelScore;
    bonusEndElement.classList.remove('fading');
    void bonusEndElement.offsetWidth;
    bonusEndElement.classList.add('active');
    setTimeout(function() {
      if (onHoldDone) onHoldDone();
    }, 1800);
  }
  function hideBonusEndedSplash() {
    bonusEndElement.classList.add('fading');
    setTimeout(function() {
  ...
  // The full bonus-end visual sequence. Called the moment a falling hammo is
  // detected. Runs as a setTimeout chain so each phase has explicit duration:
  //   A. Stop the Runner. Start the canvas zoom transition. Draw circle.
  //   B. After ZOOM_DURATION_MS + POST_ZOOM_PAUSE_MS: snap zoom back, clear
  //      the world, show the bonus-ended splash.
  //   C. Show the next-level intro under the bonus splash, fade the bonus
  //      splash away so the level-intro takes over without revealing canvas.
  function startBonusEndSequence(fh) {
    Runner.stop(runner);
    // Hide the HUD overlays (welcome badge, shots, score) since they live
    // outside the canvas and don't scale with the zoom.
    canvasWrapper.classList.add('frozen');
    // Zoom the canvas element itself so the wrapper's outer size stays fixed.
    render.canvas.style.transformOrigin =
      fh.position.x + 'px ' + fh.position.y + 'px';
    render.canvas.style.transform = 'scale(1.3)';
    // Force one render so the red circle and frozen positions display even
    // though the Runner is stopped.
    Render.world(render);

    setTimeout(function() {
      // End of Phase A + B: lock score, stop drawing circle, snap zoom out.
      ...
      hideBonusTimer();
      fallingHammo = null;
      // Snap (no transition) the canvas back to normal so the splash shows
      // at canvas dimensions, not zoomed dimensions. Also drop the .frozen
      // class so the HUD overlays come back for the post-splash game view.
      canvasWrapper.classList.add('zoom-snap');
      render.canvas.style.transform = '';
      render.canvas.style.transformOrigin = '';
      void render.canvas.offsetWidth;
      canvasWrapper.classList.remove('zoom-snap');
      canvasWrapper.classList.remove('frozen');
      // Wipe the world so the post-splash "regular game view" pause shows a
      // clean slate, not the frozen chaos.
      World.clear(engine.world, false);
      hammos = [];

      // Restart the Runner. Compute totals. Set pickingNew so the engine tick
      // early-returns through the transition. The Runner is needed because
      // the Render loop independently keeps drawing the canvas, but with
      // pickingNew = true our game logic doesn't fire.
      Runner.run(runner, engine);
      ...
      // Show the bonus splash (z-index 160) for 1800ms. Then start the
      // next-level intro (z-index 150) underneath, and fade the bonus splash
      // away, revealing the level-intro entry animation as it plays through.
      showBonusEndedSplash(liveScore, function() {
        ...
        // Fade out the bonus splash after a brief moment so the level-intro
        // underneath has begun its staggered text reveal as the splash fades.
        setTimeout(hideBonusEndedSplash, 100);
      });
    }, ZOOM_DURATION_MS + POST_ZOOM_PAUSE_MS);
  }
